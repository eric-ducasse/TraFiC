...
class USMultilayerPipe:
    ...
    def nMBytes(self):
        ...
        if self.M is not None:
            mbs += USMultilayerPipe.TWOPOWM20*self.M.nbytes
        return mbs    
    @property
    ...
    def prependLayer(self, r_min, material, angles=None):
        if ( r_min < 1e-5
             and ( isinstance(material, Fluid)
                   or isinstance(material, IsotropicElasticSolid)
                   or isinstance(material,
    ...
    def update(self, Vs, Vk, Vn=None, usmat="All", buildM=True):
        if ( (self.__Vs is not Vs) or (self.__Vk is not Vk)
             or (self.__Vn is not Vn) ): # detected changes
            usmat="All"
        axisym = (Vn is None)
        if usmat is not None: # one or more US material to update
            ...
            if axisym: # axisymmetrical
                ...
                S2,K,N = np.meshgrid(self.__Vs**2, self.__Vk,
                                     self.__Vn, indexing="ij")
                K2 = K**2
                if usmat == "All":
                    for usm in self.__Lusm:
...
##        print("update -> dim:",d,"; cols:",cols,"; rows:",rows)
        ...
        for out_lay,cmax,rmax in zip( self.layers[1:],
                                      self.n_cols[2:],
                                      self.n_rows[1:] ):
            ...
            if in_lay.npw == 6:
                if out_lay.npw == 6: # Solid/Solid
                    ...
                    # Fixed 2019 Dec., 08:
                    self.__M[...,rmin+1,cmil:cmax] = -FCout[...,-1,:]
                                                                    # Srr
            else: # in_lay.npw == 2
    ...
    def __str__(self):
        ...
        if len(self.layers) == 0:
            ...
            msg += ( "\n" + dbb + f" Interface #{n+1} ({self.dim-noeq0} "
                     + f"equations) at r = {1e3*self.R[-1]:.2f} mm "
                     + dbb )
        # External space
    ...
    def export_to_text(layer_data, materials, InCyl, ExtSp):  
        ...
        if nb_mat>1: text += "s"
        text += ( "\n" + dline + "\n"
                  + f"Material of inner cylinder of {r_str}: "
                  + incylmat+ "\n" + sline + "\n" )
        for i,(w,m) in enumerate(layer_data,1):
            text += f"Layer {i}:\n\t{w}\n\t{m}\n" + sline + "\n"
        r_str, extmat = ExtSp
        text += ( f"External {r_str}\n"
                  + "Material of external space: " + extmat
                  + "\n" + dline + "\n" )
        if len(materials) > 0:
...
if __name__ == "__main__":
    ...
    test1 = USMultilayerStructure( data_path + "Plates/test_23-06-09.txt" )
    print( test1.write_in_file() )
    test2 = USMultilayerStructure( data_path + "Pipes/test_23-07-05.txt" )
    ...
    #####################
    water_inside = True
    if water_inside: pipe1.setInnerCylinder(water)
    #####################
    print(pipe1)
    file_pth = data_path + "Pipes/test_23-07-06.txt"
