...
class USLayer:
    ...
    def __str__(self, number=None):
        piece = "" if number is None else f" #{number}"
        msg = ( "Layer" + piece + f" of {self.usm.mat.name} and of "
                + f"thickness {self.h*1e3:.2f} mm "
...
class USMultilayerPlate:
    ...
    def update(self, Vs, Vkx, Vky=None, usmat="All", buildM=True):
        if ( (self.__Vs is not Vs) or (self.__Vkx is not Vkx)
             or (self.__Vky is not Vky) ): # detected changes
            usmat="All"
        twoD = Vky is None
        if usmat is not None: # one or more US material to update
            ...
            if twoD: # 2D
                ...
                S2,Kx,Ky = meshgrid(self.__Vs**2, self.__Vkx,
                                    self.__Vky, indexing="ij")
                Kx2,Ky2 = Kx**2,Ky**2
                if usmat == "All":
...
##        print("update -> dim:",d,"; C:",C,"; R:",R)
    ...
    def fields(self,z,Vc,wanted="all",output = "list", tol_pos=1e-9):
        ...
        if wanted == "all":
            ...
            msg = ( "USMultilayerPlate.fields:: unrecognized\n"
                    + f"value '{wanted}' for wanted" )
            raise ValueError(msg)
        else:
            for F in wanted:
                if F not in choices:
                    msg = ( "USMultilayerPlate.fields:: unrecognized\n"
                            + f"value '{F}' in {wanted} for wanted" )
                    raise ValueError(msg)       
        if (self.__Vs is None) or (self.__Vkx is None):
        ...
        if z < 0: # top half-space
            ...
            if usm is None or usm == "wall": # Vacuum
                print(f"Error: z = {1e3*z:.6f} mm is in vacuum")
                return None
            npw = usm.nb_pw
            if npw != C[0]:
                print(f"Error in top half-space: {npw} != {C[0]}")
                return None
        ...
        # Layer in the plate
            ...
            if search_layer:
                ...
                npw = usm.nb_pw
                if npw != self.dim-C[-1]:
                    print(f"Error in bottom half-space: {npw} != "
                          + f"{self.dim-C[-1]}")
                    return None
                CC = np.zeros( self.shape+(2*npw,), dtype = np.complex128)
    ...
    def __str__(self):
        ...
        dbb = 18*"="
        msg = ( stars + "\n"
                + f"{len(self.layers)}-layered plate of thickness "
                + f"{1e3*self.Z[-1]:.2f} mm and with "
        ...
        if self.__bhs is None:
            ...
            bhs = ( self.__bhs.mat.name + f" ({npw} partial downgoing "
                    + "wave{})".format("s" if npw > 1 else "") )
        msg += f"\n Bottom Half-Space: {bhs}\n" + stars
        return msg
    #-----------------------------------------------------------------------
    def write_in_file(self, file_path=None):
        ...
        for lay in self.layers:
            ...
            elif w_mm >= 0.01: # small thicknesses added in v1.73
