...
class ComputationParameters:
    ...
    RND_X = 1.0e-7 # Discretization step(s) rounded to the nearest
                   # multiple of RDN_X, in meters, if possible
    ...
    def __init__(self, label, duration_µs, delay_µs, max_frequency_MHz,
                 ...
                 round_space_step=True, nx=None, ny=None
                 ):
        if verbose:
            self.__prt = print
        else:
        ...
        if self.__3d:
            if structure == "plate":
                if round_space_step:
                    dx, dy = [ rnd_x * np.floor(
                                    0.5e-3*float(v) / rnd_x + 1e-4 )
                               for v in min_wavelength_mm ]
                else:
                    dx, dy = [0.5e-3*float(v)
                              for v in min_wavelength_mm ]
                # Number of discretization points
                # New in V1.12: forced nx and ny
                if nx is None:
                    nx = 2*round(np.ceil( x_max / dx ))
                new_x_max = 0.5*nx*dx
                if abs(new_x_max-x_max) > 0.5*rnd_x: # too rounded
                    dx = 2*x_max / nx
                if ny is None:
                    ny = 2*round(np.ceil( y_max / dy ))
                new_y_max = 0.5*ny*dy
                if abs(new_y_max-y_max) > 0.5*rnd_x: # too rounded
                    dy = 2*y_max / ny
            else: # structure == "pipe"
                lbdax, lbday = min_wavelength_mm
                if round_space_step:
                    dx = rnd_x * np.floor(
                                    0.5e-3*float(lbdax)/rnd_x + 1e-4)
                else:
                    dx = 0.5e-3*float(lbdax)
                # Number of discretization points
                # New in V1.12: forced nx and ny
                if nx is None:
                    nx = 2*round(np.ceil( x_max / dx ))
            ...
            if round_space_step:
                dx = rnd_x * np.floor(
                        0.5e-3*float(min_wavelength_mm)/rnd_x + 1e-4)
            else:
                dx = 0.5e-3*float(min_wavelength_mm)
            # Number of discretization points
            # New in V1.12: forced nx
            if nx is None:
                nx = 2*round(np.ceil( x_max / dx ))
            new_x_max = 0.5*nx*dx
            if abs(new_x_max-x_max) > 0.5*rnd_x: # too rounded
        ...
        if structure == "plate":
            if plate_path == "auto":
                plate_path = os.path.join(self.ROOT, "Data",
                                          "Plates", "")
        ...
        if self.__3d:
            if self.__struct == "plate":
                txt += (f"2D Space grid: {self.nx} x {self.ny} nodes"
                        ...
                        + f"dx: {self.dx_mm:.5f} mm\n\t"
                        + f"ymax: {self.ymax_mm:.3f} mm\n\t"
                        + f"dy: {self.dy_mm:.5f} mm\n")
            else: # self.__struct == "pipe":
                rrf = 1e3*self.__ref_radius_m
                txt += (f"2D Space grid: {self.nx} x {self.ny} nodes"
                        + f"\n\tzmax: {self.xmax_mm:.3f} mm\n\t"
                        + f"dz: {self.dx_mm:.5f} mm\n\t"
                        + f"nmax: {self.ny//2} \n\t"
                        + f"da: {self.da_degree:.6f}° "
                        + f"({self.dy_mm:.5f} mm for the ref. "
                        + "radius)\n\t"
