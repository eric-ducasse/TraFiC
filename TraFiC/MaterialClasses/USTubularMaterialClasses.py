# Version 1.31 - 2026, Marsh, 18
# Copyright (Eric Ducasse 2020, coll. Aditya Krishna)
...
class USTubularFluid(USTubularMat):
    ...
    def field_components(self, r, r0, S2, K, N=None):
        ...
        if N is None: # Axisymmetrical            
            ...
            # Fixed in v1.31: negative values of N
            Nm1, N0, Np1 = np.abs(N-1), np.abs(N), np.abs(N+1)
            Km1,K0,Kp1 = (np.einsum(self.SSprod,Eplus,kve(Nm1,Eta_r)),
                          np.einsum(self.SSprod,Eplus,kve(N0,Eta_r)),
                          np.einsum(self.SSprod,Eplus,kve(Np1,Eta_r)))
            Im1,I0,Ip1 = (np.einsum(self.SSprod,Emoins,ive(Nm1,Eta_r)),
                          np.einsum(self.SSprod,Emoins,ive(N0,Eta_r)),
                          np.einsum(self.SSprod,Emoins,ive(Np1,Eta_r)))
            ...
            return FC
    #-----------------------------------------------------------------------
    def sigmaThetaTheta(self, U, r=None, K=None, N=None):
                               # r,K,N not used in a fluid
        """Returns the stress Saa = Srr = Szz = -pressure,
           from the 4-dimensional field U."""
        return U[...,3]
    #-----------------------------------------------------------------------
    def sigmaZZ(self, U, r=None, K=None, N=None):
                               # r,K,N not used in a fluid
    ...
    def sigmaThetaZ(self, U, r=None, K=None, N=None):
        """Returns the stress Saz = 0, from the 4-dimensional field U."""
...
class USTubularTransverselyIsotropicElastic( USTubularMat ):
    ...
    def update(self, S2, K, K2, N=None):
        ...
        if N is not None: # Eta and coefficients independent from n
            S2,K,K2 = S2[:,:,0],K[:,:,0],K2[:,:,0]
        Vk = K[0,:]
        K_is_zero = np.abs(Vk) < 1e-10 # k values considered as zero
        ...
        test = np.abs(denom1) > np.abs(denom2)
        b1 = np.empty_like(S2)
        idx = np.where(test)
        b1[idx] = (m.c11*Eta1_square[idx]-a44[idx])/denom1[idx]
        test0 = (~test)&(denom2==0.0)
        ...
        if nb0 != len(self.__idx_k_eq_0)*shp[0]: # Should be for k=0 only
            print("Case 0 for eta2:", len(idx[0]), "/", S2.size,
                  f"!= {len(self.__idx_k_eq_0)}x{shp[0]}")
            print("(S,K):")
            print(np.array([sqrt(S2[idx]),K[idx]]).T)
    ...
    def field_components(self, r, r0, S2, K, N=None):
        ...
        if N is None: # Axisymmetrical            
            ...
            idx = 'ijl,ijkl->ijkl'
            # Fixed in v1.31: negative values of N
            N3m2,N3m1,N30,N3p1,N3p2 = [np.abs(N3+d) for d in (-2,-1,0,1,2)]
            Km1,K0,Kp1 = (np.einsum(idx, Eplus, kve(N3m1,Eta_r)),
                          np.einsum(idx, Eplus, kve(N30,Eta_r)),
                          np.einsum(idx, Eplus, kve(N3p1,Eta_r)))
            Km2,Kp2 = (np.einsum(idx, Eplus, kve(N3m2,Eta_r)),
                       np.einsum(idx, Eplus, kve(N3p2,Eta_r)))
            Im1,I0,Ip1 = (np.einsum(idx, Emoins, ive(N3m1,Eta_r)),
                          np.einsum(idx, Emoins, ive(N30,Eta_r)),
                          np.einsum(idx, Emoins, ive(N3p1,Eta_r)))
            Im2,Ip2 = (np.einsum(idx, Emoins, ive(N3m2,Eta_r)),
                       np.einsum(idx, Emoins, ive(N3p2,Eta_r)))
            # Displacements
            ...
            FC[...,3:,:3] = ( self.mat.c66
                              *( np.einsum("ijkm,lm->ijklm",
                                           np.einsum(idx,self.Eta,Km2),
                                           Mm)
                                 + np.einsum("ijkm,lm->ijklm",
                                             np.einsum(idx,self.Eta,Kp2),
                                             Mp) ) )
            ...
            FC[...,3:,-3:] = ( self.mat.c66
                               * (np.einsum("ijkm,lm->ijklm",
                                            np.einsum(idx,self.Eta,Im2),
                                            Mm)
                                  + np.einsum("ijkm,lm->ijklm",
                                              np.einsum(idx,self.Eta,Ip2),
                                              Mp) ) )
            FC[...,3,3] += C0*I0[...,0]
            FC[...,3,4] += C1*I0[...,1]
            ...
            for no in self.__idx_k_eq_0: # 0 or 1 value
                ...
                FC[:,no,:,2,1] = K0[:,no,:,1] 
                FC[:,no,:,2,4] = I0[:,no,:,1] 
                FC[:,no,:,5,1] = ( -0.5*self.mat.c44
                                   * np.einsum("i,ij->ij",
                                               self.Eta[:,no,1],
                                               Km1[:,no,:,1]+Kp1[:,no,:,1])
                                   ) 
                FC[:,no,:,5,4] = ( 0.5*self.mat.c44
                                   * np.einsum("i,ij->ij",
                                               self.Eta[:,no,1],
                                               Im1[:,no,:,1]+Ip1[:,no,:,1])
                                   )
...
if __name__ == "__main__":
    ...
    crbepx = TransverselyIsotropicElasticSolid({
                "rho":1560.0, "c11":1.4e10, "c12": 8.0e9, "c33": 8.7e10,
    ...
    Vk,Vn = gd.Kx, gd.Ky
    print(f"Vk ~ {Vk.round(2)}")
    print(f"Vn = {Vn.round(2)}")
